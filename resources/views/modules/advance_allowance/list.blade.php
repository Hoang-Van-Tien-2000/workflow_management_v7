...
@include('modules.advance_allowance.create-modal')
@include('modules.advance_allowance.update-modal')
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-12 mb-4">
                            <div class="row">
                                <div class="col-3">
                                    <select  multiple="multiple" class="form-control  basic" id="fullname" name="fullname">
                                        @foreach($users as $user)   
                                            <option value="{{$user->id}}">{{$user->fullname}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @if(Auth::user()->hasPermissionTo("Thêm tạm ứng, phụ cấp"))
                                <div class="col-9">
                        ...
                        @if(Auth::user()->hasPermissionTo("Xóa tạm ứng, phụ cấp"))
                        <div class="col-12 mb-4">
                            <div class="row">
                                <div class="col-3">
                        ...
                        <div class="col-12">
                            <div class="statbox widget box box-shadow">
                                <div class="widget-content widget-content-area">
                                    <table id="advance_allowance" class="table style-2  table-hover">
                                        <thead>
                                            <tr>
...
<script>
function myFunction() {
  var x=document.getElementById("advance").value;
  if(isNaN(x))
  {
    document.getElementById("advance").value="";
  }
  var y=document.getElementById("allowance").value;
  if(isNaN(y))
  {
...
<script type="text/javascript">
    $("#fullname").multipleSelect({
        ...
        onClose: function () {
            var filterName = JSON.stringify($("#fullname").val())
...
<script>
    ...
    resetTable()
    function resetTable() {
        table = $('#advance_allowance').DataTable({
            processing  : true,
            serverSide  : true,
            autoWidth   : false,
            pageLength  : 10,
            ...
            ajax: {
                url: "{{route('advance_allowance.load_ajax_list_advance_allowance')}}",
                type: 'get'
            },
            select: {
                style: 'multi',
            ...
            initComplete: function(settings, json) {
                $('#advance_allowance tbody').on('click', 'input[type="checkbox"]', function(e) {
            ...
            columns: [
                ...
                { name: 'fullname', defaultContent: '',data: 'fullname',visible: false,bSortable: true},
            ],
            columnDefs:[ 
                {
                    'targets': 0,
    ...
    $(document).ready(function() {
        ...
        $('#btn-save-modal').click(function() {
            if($('#frm-them-moi').parsley().validate()) {
               $.ajax({
                    url: "{{ route('advance_allowance.store') }}",
                    type: 'POST',
        ...
        $('#btn-update-modal').click(function(e) {
            if($('#frm-cap-nhat').parsley().validate()) {
                $.ajax({
                    url: "{{ route('advance_allowance.update') }}",
                    type: 'POST',
...
<script type="text/javascript"> 
    function updateRow(a) {
        var id = $(a).data("id");
        var advance = $(a).data("advance");
        var allowance = $(a).data("allowance");
        var user_id = $(a).data("user_id");
        var modal = $("#update-modal");
        modal.modal('show')
        modal.on('shown.bs.modal', function(e) {
             $(e.currentTarget).find('input[name="id"]').val(id);
             $(e.currentTarget).find('input[name="allowance"]').val(allowance);
             $(e.currentTarget).find('input[name="advance"]').val(advance);
             $('#user_id_update').val(user_id);
		     $('#user_id_update').select2().trigger('change');
        });
        modal.on('hidden.bs.modal', function() {
...
<script type="text/javascript">
    function deleteRow(a) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "{{route('advance_allowance.destroy')}}",
                    type: 'post',
                    data: {id:id},
                }).done(function(res) {
                    if (res.status == 'success') {
