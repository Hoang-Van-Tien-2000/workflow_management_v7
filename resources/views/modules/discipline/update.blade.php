            ...
            <div class="card">
                <div class="card-body">
                    <form method="POST" id="frm-them-moi" data-parsley-validate="" novalidate>
                        @csrf
                        <div class="row">
                            <input type="hidden" class="form-control" name="id" id="id" value="{{$discipline->id}}">
                        ...
                        <div class="row">
                            <div class="col-md-6 col-sm-12" >
                                ...
                                <input type="text" class="form-control" name="salary_fine" id="salary_fine"
                                oninput="myFunction()"
                                onkeypress="return isNumberKey(event)"
                                value="{{$discipline->salary_fine}}"
                                placeholder="Nhập lương phạt"
                                data-parsley-required-message="Vui lòng nhập lương phạt"
                                data-parsley-maxlength="20"
                                data-parsley-maxlength-message="Lương phạt không thể nhập quá 20 kí tự"
                                required>
                            </div>
                        </div>
...
<script>
function myFunction() {
  var x=document.getElementById("salary_fine").value;
  if(isNaN(x))
  {
    document.getElementById("salary_fine").value="";
  }
}
</script>
<script>
    $('#btn-submit-form').click(function() {
        if($('#frm-them-moi').parsley().validate()) {
            var formData = new FormData();
                ...
                $("input[name='salary_fine']").map(function(){ formData.append('salary_fine', this.value)}).get();
